...
class SecretSantaCoreVlad {
	 ...
	 * checked whether the e-mail address is correct and that all of them are unique.
	 *
	 * @param $users
	 *
	 * @return bool
	...
	public function goRudolph() {
		if ( $this->validateData() == true ) {
			...
			for ( $i = 0; $i < count( $this->users ); $i ++ ) {
				$message = $this->users[ $i ]['name'] . ',\r\n\r\nThe Christmas is coming and for the Secret Santa event you have to buy a present for ' . $this->users[ $paired_users[ $i ] ]['name'] .
				           '. Also, the recommended value of the gift is ' . $this->recommendedExpenses . ' Merry Christmas!';
